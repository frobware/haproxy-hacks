...
func (crw *customResponseWriter) WriteHeader(statusCode int) {
	...
	crw.logger.Logf("%s -> %s: %s", crw.conn.LocalAddr(), crw.conn.RemoteAddr(), statusLine)
	...
	for _, k := range keys {
		for _, val := range crw.headers[k] {
			...
			crw.logger.Logf("%s -> %s: %s", crw.conn.LocalAddr(), crw.conn.RemoteAddr(), headerLine)
	...
	crw.logger.Logf("%s -> %s: %s", crw.conn.LocalAddr(), crw.conn.RemoteAddr(), "\r\n")
}
...
func (crw *customResponseWriter) Write(data []byte) (int, error) {
	...
	if crw.haveTransferEncodingHeader() {
		...
		crw.logger.Logf("%s -> %s: Chunk-Size: %s", crw.conn.LocalAddr(), crw.conn.RemoteAddr(), chunkSize)
		crw.logger.Logf("%s -> %s: Chunk-Data: %q", crw.conn.LocalAddr(), crw.conn.RemoteAddr(), string(data))
	...
	crw.logger.Logf("%s -> %s: Body: %q", crw.conn.LocalAddr(), crw.conn.RemoteAddr(), string(data))
...
func (crw *customResponseWriter) recordWriteError(err error, bytesWritten int, content string) {
	...
	crw.logger.Logf("%s -> %s: %s", crw.conn.LocalAddr(), crw.conn.RemoteAddr(), errorMessage)
...
func (crw *customResponseWriter) finaliseChunking() (int, error) {
	...
	crw.logger.Logf("%s -> %s: Chunk-Size: 0", crw.conn.LocalAddr(), crw.conn.RemoteAddr())
	crw.logger.Logf("%s -> %s: End-Of-Chunked-Transfer", crw.conn.LocalAddr(), crw.conn.RemoteAddr())
