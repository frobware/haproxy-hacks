...
func (w *duplicateTEResponseWriter) Write(body []byte) (int, error) {
	...
	fmt.Fprintf(w.bufRW, "%x\r\n", len(body)) // Write the size of the chunk in hex
	n, err := w.bufRW.Write(body)             // Write the chunk data
	if err != nil {
		return n, err
	}
...
func (w *duplicateTEResponseWriter) Close() error {
	_, err := fmt.Fprintf(w.bufRW, "0\r\n\r\n") // Write the final 0-length chunk
	return err
}
...
func duplicateTEHandler(w http.ResponseWriter, r *http.Request) {
	...
	// Manually set multiple Transfer-Encoding headers.
...
func main() {
	...
	http.HandleFunc("/discovery", func(w http.ResponseWriter, req *http.Request) {
		fmt.Fprint(w, "/single-te /duplicate-te /healthz")
	})
